...
class Target(object):
    ...
    def __init__(self,hardware):
        ...
        self.CFE_HARD = ["arcadyan-starlite","askey-stargatev2","siligence-cut2"]
    ...
    def get_build(self,path,build):
        ...
        if not file.is_file():
            ...
            #print("sudo wget -P {0} {1}{2} 2>/dev/null 1>&2".format(dir,path,build.replace("#", "%23")))
    ...
    def is_logged(self):
        ...
        data = self.serial_read()
        print(data)
        bufferis += data.strip().decode('utf-8')
        print(bufferis)
...
def firmware_upgrade(path,build):
    ...
    target.add_target(serial_port='/dev/ttyUSB0', lan_intf='enp0s8')
...
"""
target = Target("askey-stargatev2")
print(target.hardware)
path = "http://192.168.3.114/"
build = "2018-04-23_V10.00.04#00_swisscom_stargatev2_askey-stargatev2_swisscom_multi_sip_normal_bcmflash.img"
firmware_upgrade(path,build)
print ('script done')
sys.exit()
"""
